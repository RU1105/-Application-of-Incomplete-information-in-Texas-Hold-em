...
print("reading file...")
with open(r'D:\porker_cnn\完成\x.csv', newline='') as csvfile:
    # 讀取 CSV 檔內容，將每一列轉成一個 dictionary
    #rows = csv.DictReader(csvfile) #csv.DictReader可以資料查找 d = {key1: value1, key2: value2}
    rows = csv.reader(csvfile)
    data_list = list(rows)

with open(r'D:\porker_cnn\完成\y.csv', newline='') as file:
    # 讀取 CSV 檔內容，將每一列轉成一個 dictionary
    #rows = csv.DictReader(csvfile) #csv.DictReader可以資料查找 d = {key1: value1, key2: value2}
...
file = open('D:/porker_cnn/完成/train.csv',mode='w', newline='')
