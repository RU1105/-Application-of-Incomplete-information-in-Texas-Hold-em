...
class BBQ():
    ...
    def predict(self,input):
        with open("D:/Finish/train/model.config", "r") as text_file: #bugg
            json_string = text_file.read()

        model = Sequential()
        model = model_from_json(json_string)
        model.load_weights("D:/Finish/train/model.weight", by_name=False)
        ...
        print("me predict :",predict)

    def predict_RF(self,input):
        model = joblib.load("D:/Finish/randomF/my_random_forest.joblib")
        output = int(float(model.predict(np.array(input).reshape(1, -1))))
        print("學長predict :",output)
        return output
    ...
    def card_strength2(self,card):
        ...
        if(ranks_type_keys == -1):
            if len(rank_sorted)>0:
                ranks_type_keys=rank_sorted[len(rank_sorted)-1]       
        #-------OUTPUT----------------
        #type 0高牌 1一對 2兩對 3三條 4順子 5同花 6葫蘆 7鐵支 8同花順 9同順缺1 10同花缺1 11同花缺2 12順缺1
        if(ranks_type == 4):#鐵支
            type.append(7)
