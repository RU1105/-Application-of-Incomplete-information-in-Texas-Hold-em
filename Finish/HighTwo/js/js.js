...
function init_game(){
	...
	//將電腦的牌蓋回去
	$(".game_table_content > div > .card_wrap[card_type=" + 'enemy_card1' + "]").addClass('active');
	$(".game_table_content > div > .card_wrap[card_type=" + 'enemy_card2' + "]").addClass('active');
...
function call_chip(){
	...
	if(turn_number == 1){
		...
		if(next_step == 'step2'){
			give_card_step2();
			console.log('step2:' + next_step)
		}
		else if(next_step == 'step3'){
			give_card_step3();
			console.log('step3:' + next_step)
		}
		else if(next_step == 'step4'){
			give_card_step4();
			console.log('step4:' + next_step)
		}
...
function my_data_post(){
	...
	var input_test = [
		...
		all_card_value['my_card1'], all_card_value['my_card2'], //電腦的手排
		my_card_level,	// 接手牌的level值
	...
	console.log('RF post = ' + post_input);
	post_input.splice(14, 0, 1);//學長
		  //下面傳送
		  $.ajax({
			type:'POST',
			...
			{
				setTimeout(function(){
					console.log('RF receive = ' + data);
					if(turn_number <= 2){
						my_card_level = data[2];
					}
					var raise = data[1];
					data = data[0];
					if(data == 2){ //修all-in後，又加注會顯示加注為0的情況，改為call或check
						if(my_chip_remind == my_chip_value){
							data = 3
						}
					}
					if(data == 2){	//加注
						if(raise + my_chip_value > my_chip_remind){  //(加注金額 + 玩家的牌桌籌碼) > 玩家的籌碼剩餘 -> all-in
							what_RF_do('All in ♥');
						}
						else{
							what_RF_do('Raise ' + parseInt(raise).toString());
						}
...
function enemy_data_post(){
	...
	console.log('CNN post = ' + post_input);
	post_input.splice(14, 0, 0);//ME
		  //下面傳送
	$.ajax({
		type:'POST',
		url:'http://127.0.0.1:8080',
		...
		success: function(data)
		{
			setTimeout(function(){
				...
				if(data == 2){	//加注 阿修!記得修這裡 // 還有bug但我找不出來
...
//讓玩家知道RF在做什麼唷♥
function what_RF_do(text){
	$('.my_action').html(text);
	$('.my_action').addClass('active');
	setTimeout(function(){
		$('.my_action').removeClass('active');
	}, 1500);
}
