...
import os
import numpy as np
path1 = r"D:\Finish_2\BBQ" #當前上一層目錄的絕對路徑，os.path.abspath('..') 隨機選20萬 AI對打
sys.path.insert(1, path1)
import BBQ

FILE = r'D:\Finish_2\HighTwo\welcome.html' """記得改回原本的路徑"""
PORT = 8080
BBQ = BBQ.BBQ()
action=0
        ...
        try:
            ...
            if (type=="predict") :
                ...
                if input[7] == 0: #第一階段
                    ...
                    print("---第二階段")
                    #測試用input = [51,44,25,11,-1,26,45,4,2915,2915,-1,-1,-1,-1]
...
def abandon(input):#包含棄牌與all-in
    ...
    #順序大小0 < 1 < 3 < 2 < 4 < 5 < 6 < 7 < 8
        ...
        if input[8] == input[9]: #判斷是否有加注(check or call)
            action+=1
        #print("action: ", action)
        if strength == 2: #兩對 經過測試兩對的ALL-IN最好不要太高機率
            if action==1: 
...
def action_predict(input):
    ...
    for j in range(15):
        ...
        if j==10: #小數籌碼
            ...
            continue
        if j>=11 and j<=14:
            if int(action_list[11])==0 and int(action_list[12])==1 and int(action_list[13])==1 and int(action_list[14])==0:
                size[11, 0:4] = [1,1,1,1]
            elif int(action_list[11])==-1 and int(action_list[12])==-1 and int(action_list[13])==-1 and int(action_list[14])==-1:
                size[11, 4:8] = [1,1,1,1]
            elif int(action_list[11])==0 and int(action_list[12])==0 and int(action_list[13])==1 and int(action_list[14])==1:
    ...
    new_input.append(0)
    print("new input :",new_input) #改一下有muck
...
def hand_strength(hands): #阿修阿修 這邊有bug 同花跟同花順跟順子判斷有問題
    ...
    if(ranks_type == 4):
        ...
        print(ranks_type_keys[0])
        hands_type = [1,ranks_type_keys[0],max(hands[0:2])] #這邊規則出問題了，當強度、pair都一樣時，要比雙方的hand card，已修改
